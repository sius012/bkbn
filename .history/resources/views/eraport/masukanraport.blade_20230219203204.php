   ...
   @foreach ($kls as $item)
       ...
       <div class="row mt-1">
        <div class="col"><button class="btn btn-primary-bb btn-sm btn-kelas" value='{{$item["angkatan"].":".$item["jurusan"]}}'' data-toggle="modal" data-target="#modal-input-raport">Input Raport</button>
            <a href=""><button class="btn btn-secondary btn-sm">E-Raport</button></a>
            <a  href="{{url('daftar-raport-siswa/'.$item["angkatan"]."/".$item['jurusan'])}}">
            <a href=""></a><button class="btn btn-primary btn-sm"><i class="fa fa-users"></i></button></a>
        </div>
