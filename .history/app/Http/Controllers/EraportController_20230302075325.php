...
class EraportController extends Controller
{
    ...
    public function validasiraport(Request $req)
    {
        ...
        if ($permission == true) {
            $req->session()->put('dataraport', ['dataraport' => $finallyRow, "angkatan" => $angkatan->id_angkatan, "jurusan" => $jurusanril->id_jurusan, "tanggal_penilaian"]);
        } else {
    ...
    public function injectraport(Request $req)
    {
        if ($req->session()->has('dataraport')) {
           // dd($req->session()->get('dataraport'));
            ...
            $rows = $req->session()->get('dataraport')['dataraport'];
            $angkatan = $req->session()->get('dataraport')['angkatan'];
            $jurusan = $req->session()->get('dataraport')['jurusan'];
