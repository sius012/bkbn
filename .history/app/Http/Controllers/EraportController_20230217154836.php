...
class EraportController extends Controller
{
   ...
   // dd($rows);
    ...
    public function masukanraport(Request $req){
        ...
        $list = PenilaianGuru::with("detail_pg.siswa")->with(["detail_pg.aspek_dpg"=>function($q){
            $q->where("nis_siswa", 82615029);
        }])->get();
        dd($list);
   ...
   //    dd($ketarangan);
    ...
    public function detailraport($nis){
        $list = PenilaianGuru::whereHas('detail_pg', function($query) use($nis) {
            $query->where('nis_siswa',$nis);
        })->with("detail_pg.aspek_dpg")->get();
        //dd($list);
        return view("component.raport-karakter-siswa", ["listPenilaian"=>$list]);
    }
}
