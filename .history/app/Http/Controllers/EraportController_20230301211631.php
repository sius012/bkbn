...
class EraportController extends Controller
{
   ...
   // dd($rows);
    ...
    public function validasiraport(Request $req){
        ...
        $tablevalidasi["Rentang Waktu Penilaian"] = PenilaianGuru::whereMonth("tanggal_penilaian", date("m", strtotime($req->tanggal)))->whereYear("tanggal_penilaian",date("Y", strtotime($req->tanggal)))->where("id_penilai", Auth::user()->id)->where("id_angkatan", $param[0])->where("id_jurusan", $jurusanril)->count();
