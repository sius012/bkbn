...
class EraportController extends Controller
{
   ...
   // dd($rows);
    ...
    public function validasiraport(Request $req){
        ...
        $tablevalidasi["Rentang Waktu Penilaian"] = PenilaianGuru::whereMonth("tanggal_penilaian", date("m", strtotime($req->tanggal)))->where("id_penilai", Auth::user()->id)->where("kelas", $kelasril)->where("jurusan", $jurusan);
